...
def replace_matrix(matrix_a, matrix_b, row):
    matrix_a[matrix_b[row, 0],
             matrix_b[row, 1]] = 0
    matrix_a[matrix_b[row, 1],
             matrix_b[row, 0]] = 0


def cross_validation(drug_drug_matrix, CV_num, seed, file_results_path, weights_results_path):
    link_number = 0
    link_position = []
    ...
    for CV in range(0, CV_num):
        ...
        for i in range(0, len(testLinkPosition)):
            train_drug_drug_matrix[testLinkPosition[i, 0],
                                   testLinkPosition[i, 1]] = 0
            train_drug_drug_matrix[testLinkPosition[i, 1],
                                   testLinkPosition[i, 0]] = 0
            testPosition = list(testLinkPosition) + list(nonLinksPosition)
            # testPosition = np.concatenate((testLinkPosition, nonLinksPosition))
        #  GA
        # weights, cf1, cf2 = internal_determine_parameter(
        #     copy.deepcopy(train_drug_drug_matrix))
        weights, cf1, cf2 = internal_determine_parameter(
        ...
        for i in range(0, len(multiple_predict_results)):
            [auc_score, aupr_score, precision, recall,
                accuracy, f] = multiple_predict_results[i]

            write_to_file(file_results_path, ' '.join(
        ...
        [auc_score, aupr_score, precision, recall,
            accuracy, f] = ensemble_results_cf1
        write_to_file(file_results_path, ' '.join(
            [auc_score, aupr_score, precision, recall, accuracy, f]))

        [auc_score, aupr_score, precision, recall,
            accuracy, f] = ensemble_results_cf2
        write_to_file(file_results_path, ' '.join(
            [auc_score, aupr_score, precision, recall, accuracy, f]))

        print(weights)
        weights_str = ''
        # weights_str = ''.join([x for x in weights])
        for i in range(0, len(weights)):
            weights_str = weights_str+' '+str(weights[i])
        write_to_file(weights_results_path, weights_str)
...
class Topology:
    def topology_similarity_matrix(drug_drug_matrix):
        ...
        Katz_similarity_matrix = inv(np.identity(
            drug_num)-beta*drug_drug_matrix)-np.identity(drug_num)
        for i in range(0, drug_num):
            for j in range(i+1, drug_num):
                ...
                for k in range(0, len(commonn_neighbor)):
                    le = len([x for x in (G.neighbors(commonn_neighbor[k]))])
                    AA_score = AA_score+1 / \
                        math.log(le)
                    RA_score = RA_score+1/le
        ...
        RWR_similarity_matrix = RWR_similarity_matrix + \
            np.transpose(RWR_similarity_matrix)
...
def modelEvaluation(real_matrix, predict_matrix, testPosition, featurename):
    ...
    for i in range(0, len(testPosition)):
        ...
        predicted_probability.append(
            predict_matrix[testPosition[i][0], testPosition[i][1]])
    ...
    precision, recall, pr_thresholds = precision_recall_curve(
        real_labels, predicted_probability)
    aupr_score = auc(recall, precision)

    all_F_measure = np.zeros(len(pr_thresholds))
    for k in range(0, len(pr_thresholds)):
        if (precision[k]+precision[k]) > 0:
            all_F_measure[k] = 2*precision[k] * \
    ...
    auc_score, aupr_score, precision, recall, accuracy, f = ("%.4f" % auc_score), (
        "%.4f" % aupr_score), ("%.4f" % precision), ("%.4f" % recall), ("%.4f" % accuracy), ("%.4f" % f)
...
def fitFunction(individual, parameter1, parameter2):
    ...
    for i in range(0, len(multiple_prediction)):
        ...
        #     individual[i]*multiple_prediction[i]
        numpy.append(ensemble_prediction, individual[i]*multiple_prediction[i])
    precision, recall, pr_thresholds = precision_recall_curve(
...
class MethodHub():
    def neighbor_method(similarity_matrix, train_drug_drug_matrix):
        return_matrix = np.matrix(
            train_drug_drug_matrix)*np.matrix(similarity_matrix)
    ...
    def Label_Propagation(similarity_matrix, train_drug_drug_matrix):
        ...
        transform_matrix = (1-alpha) * \
            pinv(np.identity(len(similarity_matrix))-alpha*N)
    ...
    def disturb_matrix_method(train_drug_drug_matrix):
        ...
        delta_A, row_index, col_index, select_num = MethodHub.generate_distrub_matrix(
            input_A)
        A = input_A-delta_A
        ...
        for i in range(0, num_eigenvalues):
            reconstructed_A = reconstructed_A + \
                (eigenvalues[i]+delta_eigenvalues[i]) * \
...
def ensemble_method(drug_drug_matrix, train_drug_drug_matrix, testPosition):
    ...
    chem_sim_similarity_matrix = load_csv(
        'dataset/chem_Jacarrd_sim.csv', 'float')
    target_similarity_matrix = load_csv(
        'dataset/target_Jacarrd_sim.csv', 'float')

    transporter_similarity_matrix = load_csv(
        'dataset/transporter_Jacarrd_sim.csv', 'float')
    enzyme_similarity_matrix = load_csv(
        'dataset/enzyme_Jacarrd_sim.csv', 'float')

    pathway_similarity_matrix = load_csv(
        'dataset/pathway_Jacarrd_sim.csv', 'float')
    indication_similarity_matrix = load_csv(
        'dataset/indication_Jacarrd_sim.csv', 'float')

    label_similarity_matrix = load_csv(
        'dataset/sideeffect_Jacarrd_sim.csv', 'float')
    offlabel_similarity_matrix = load_csv(
        'dataset/offsideeffect_Jacarrd_sim.csv', 'float')

    multiple_matrix = []
    multiple_result = []
    print('********************************************************')
    predict_matrix = MethodHub.neighbor_method(
        chem_sim_similarity_matrix, train_drug_drug_matrix)
    results = modelEvaluation(
        drug_drug_matrix, predict_matrix, testPosition, 'chem_neighbor')
    multiple_result.append(results)
    multiple_matrix.append(predict_matrix)

    predict_matrix = MethodHub.neighbor_method(
        target_similarity_matrix, train_drug_drug_matrix)
    results = modelEvaluation(
        drug_drug_matrix, predict_matrix, testPosition, 'target_neighbor')
    multiple_result.append(results)
    multiple_matrix.append(predict_matrix)

    predict_matrix = MethodHub.neighbor_method(
        transporter_similarity_matrix, train_drug_drug_matrix)
    results = modelEvaluation(
        drug_drug_matrix, predict_matrix, testPosition, 'transporter_neighbor')
    multiple_result.append(results)
    multiple_matrix.append(predict_matrix)

    predict_matrix = MethodHub.neighbor_method(
        enzyme_similarity_matrix, train_drug_drug_matrix)
    results = modelEvaluation(
        drug_drug_matrix, predict_matrix, testPosition, 'enzyme_neighbor')
    multiple_result.append(results)
    multiple_matrix.append(predict_matrix)

    predict_matrix = MethodHub.neighbor_method(
        pathway_similarity_matrix, train_drug_drug_matrix)
    results = modelEvaluation(
        drug_drug_matrix, predict_matrix, testPosition, 'pathway_neighbor')
    multiple_result.append(results)
    multiple_matrix.append(predict_matrix)

    predict_matrix = MethodHub.neighbor_method(
        indication_similarity_matrix, train_drug_drug_matrix)
    results = modelEvaluation(
        drug_drug_matrix, predict_matrix, testPosition, 'indication_neighbor')
    multiple_result.append(results)
    multiple_matrix.append(predict_matrix)

    predict_matrix = MethodHub.neighbor_method(
        label_similarity_matrix, train_drug_drug_matrix)
    results = modelEvaluation(
        drug_drug_matrix, predict_matrix, testPosition, 'label_neighbor')
    multiple_result.append(results)
    results = multiple_matrix.append(predict_matrix)

    predict_matrix = MethodHub.neighbor_method(
        offlabel_similarity_matrix, train_drug_drug_matrix)
    results = modelEvaluation(
        drug_drug_matrix, predict_matrix, testPosition, 'offlabel_neighbor')
    multiple_result.append(results)
    multiple_matrix.append(predict_matrix)

    # print('*************************************************************************************************************************************')
    common_similarity_matrix, AA_similarity_matrix, RA_similarity_matrix, Katz_similarity_matrix, ACT_similarity_matrix, RWR_similarity_matrix = Topology.topology_similarity_matrix(
        train_drug_drug_matrix)
    predict_matrix = MethodHub.neighbor_method(
        common_similarity_matrix, train_drug_drug_matrix)
    # predict_matrix=common_similarity_matrix
    results = modelEvaluation(
        drug_drug_matrix, predict_matrix, testPosition, 'common_similarity_neighbor')
    multiple_result.append(results)
    multiple_matrix.append(predict_matrix)

    predict_matrix = MethodHub.neighbor_method(
        AA_similarity_matrix, train_drug_drug_matrix)
    # predict_matrix=AA_similarity_matrix
    results = modelEvaluation(
        drug_drug_matrix, predict_matrix, testPosition, 'AA_similarity_neighbor')
    multiple_result.append(results)
    multiple_matrix.append(predict_matrix)

    predict_matrix = MethodHub.neighbor_method(
        RA_similarity_matrix, train_drug_drug_matrix)
    # predict_matrix=RA_similarity_matrix
    results = modelEvaluation(
        drug_drug_matrix, predict_matrix, testPosition, 'RA_similarity_neighbor')
    multiple_result.append(results)
    multiple_matrix.append(predict_matrix)

    predict_matrix = MethodHub.neighbor_method(
        Katz_similarity_matrix, train_drug_drug_matrix)
    # predict_matrix=Katz_similarity_matrix
    results = modelEvaluation(
        drug_drug_matrix, predict_matrix, testPosition, 'Katz_similarity_neighbor')
    multiple_result.append(results)
    multiple_matrix.append(predict_matrix)

    predict_matrix = MethodHub.neighbor_method(
        ACT_similarity_matrix, train_drug_drug_matrix)
    # predict_matrix=ACT_similarity_matrix
    results = modelEvaluation(
        drug_drug_matrix, predict_matrix, testPosition, 'ACT_similarity_neighbor')
    multiple_result.append(results)
    multiple_matrix.append(predict_matrix)

    predict_matrix = MethodHub.neighbor_method(
        RWR_similarity_matrix, train_drug_drug_matrix)
    # predict_matrix=RWR_similarity_matrix
    results = modelEvaluation(
        drug_drug_matrix, predict_matrix, testPosition, 'RWR_similarity_neighbor')
    multiple_result.append(results)
    multiple_matrix.append(predict_matrix)

    # print('*************************************************************************************************************************************')
    predict_matrix = MethodHub.Label_Propagation(
        chem_sim_similarity_matrix, train_drug_drug_matrix)
    results = modelEvaluation(
        drug_drug_matrix, predict_matrix, testPosition, 'chem_label')
    multiple_result.append(results)
    ...
    predict_matrix = MethodHub.Label_Propagation(
        target_similarity_matrix, train_drug_drug_matrix)
    results = modelEvaluation(
        drug_drug_matrix, predict_matrix, testPosition, 'target_label')
    multiple_result.append(results)
    multiple_matrix.append(predict_matrix)
    ...
    predict_matrix = MethodHub.Label_Propagation(
        transporter_similarity_matrix, train_drug_drug_matrix)
    results = modelEvaluation(
        drug_drug_matrix, predict_matrix, testPosition, 'transporter_label')
    multiple_result.append(results)
    multiple_matrix.append(predict_matrix)
    ...
    predict_matrix = MethodHub.Label_Propagation(
        enzyme_similarity_matrix, train_drug_drug_matrix)
    results = modelEvaluation(
        drug_drug_matrix, predict_matrix, testPosition, 'enzyme_label')
    multiple_result.append(results)
    multiple_matrix.append(predict_matrix)
    ...
    predict_matrix = MethodHub.Label_Propagation(
        pathway_similarity_matrix, train_drug_drug_matrix)
    results = modelEvaluation(
        drug_drug_matrix, predict_matrix, testPosition, 'pathway_label')
    multiple_result.append(results)
    multiple_matrix.append(predict_matrix)
    ...
    predict_matrix = MethodHub.Label_Propagation(
        indication_similarity_matrix, train_drug_drug_matrix)
    results = modelEvaluation(
        drug_drug_matrix, predict_matrix, testPosition, 'indication_label')
    multiple_result.append(results)
    multiple_matrix.append(predict_matrix)
    ...
    predict_matrix = MethodHub.Label_Propagation(
        label_similarity_matrix, train_drug_drug_matrix)
    results = modelEvaluation(
        drug_drug_matrix, predict_matrix, testPosition, 'label_label')
    multiple_result.append(results)
    multiple_matrix.append(predict_matrix)
    ...
    predict_matrix = MethodHub.Label_Propagation(
        offlabel_similarity_matrix, train_drug_drug_matrix)
    results = modelEvaluation(
        drug_drug_matrix, predict_matrix, testPosition, 'offlabel_label')
    multiple_result.append(results)
    ...
    # predict_matrix=common_similarity_matrix
    results = modelEvaluation(
        drug_drug_matrix, predict_matrix, testPosition, 'common_similarity_label')
    multiple_result.append(results)
    multiple_matrix.append(predict_matrix)
    ...
    # predict_matrix=AA_similarity_matrix
    results = modelEvaluation(
        drug_drug_matrix, predict_matrix, testPosition, 'AA_similarity_label')
    multiple_result.append(results)
    multiple_matrix.append(predict_matrix)
    ...
    # predict_matrix=RA_similarity_matrix
    results = modelEvaluation(
        drug_drug_matrix, predict_matrix, testPosition, 'RA_similarity_label')
    multiple_result.append(results)
    multiple_matrix.append(predict_matrix)
    ...
    # predict_matrix=Katz_similarity_matrix
    results = modelEvaluation(
        drug_drug_matrix, predict_matrix, testPosition, 'Katz_similarity_label')
    multiple_result.append(results)
    multiple_matrix.append(predict_matrix)
    ...
    # predict_matrix=ACT_similarity_matrix
    results = modelEvaluation(
        drug_drug_matrix, predict_matrix, testPosition, 'ACT_similarity_label')
    multiple_result.append(results)
    multiple_matrix.append(predict_matrix)
    ...
    # predict_matrix=RWR_similarity_matrix
    results = modelEvaluation(
        drug_drug_matrix, predict_matrix, testPosition, 'RWR_similarity_label')
    multiple_result.append(results)
    multiple_matrix.append(predict_matrix)
   ...
   # print('*************************************************************************************************************************************')
    ...
    results = modelEvaluation(
        drug_drug_matrix, predict_matrix, testPosition, 'disturb_matrix_label')
    multiple_result.append(results)
    multiple_matrix.append(predict_matrix)
...
def internal_determine_parameter(drug_drug_matrix):
    train_drug_drug_matrix, testPosition = holdout_by_link(
        drug_drug_matrix, 0.2, 1)
    [multiple_matrix, multiple_result] = ensemble_method(
        drug_drug_matrix, train_drug_drug_matrix, testPosition)
    weights = getParamter(drug_drug_matrix,
                          multiple_matrix, testPosition)
    # weights=[]
    ...
    for i in range(0, len(testPosition)):
        ...
        output_matrix.append(
            drug_drug_matrix[testPosition[i][0], testPosition[i][1]])
...
def ensemble_scoring(real_matrix, multiple_matrix, testPosition, weights, cf1, cf2):
    ...
    with open('real_matrix.pkl', 'wb') as f:
        pickle.dump(real_matrix, f)

    with open('multiple_matrix.pkl', 'wb') as f:
        pickle.dump(multiple_matrix, f)

    with open('testPosition.pkl', 'wb') as f:
        pickle.dump(testPosition, f)
    
    with open('weights.pkl', 'wb') as f:
        pickle.dump(weights, f)
    
    with open('cf1.pkl', 'wb') as f:
        pickle.dump(cf1, f)

    with open('cf2.pkl', 'wb') as f:
        pickle.dump(cf2, f)
    ...
    for i in range(0, len(testPosition)):
        ...
        vector = np.array(vector).reshape(-1, 29)
...
# implement 20 runs of 5-fold cross validation for base predictors and the ensmeble model
runtimes = 1
...
for seed in range(0, runtimes):
    file_results_path = "{}_{}.txt".format(file_results_str, seed)
    weights_results_path = "{}_{}.txt".format(weights_results_str, seed)
    # file_results = open(file_results_path, "w")
    # file_weights = open(weights_results_path, "w")
    cross_validation(drug_drug_matrix, 5, seed,
                     file_results_path, weights_results_path)
